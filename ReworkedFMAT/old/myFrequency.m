function frequency = myFrequency(timestamps,varargin)
...
if strcmp(method,'inverse') | strcmp(method,'iisi'),
    ...
    kernel_size = min([size(binned,1),10000]); % compute kernel size matching behavior of Smooth, but without raising a warning
	f = Smooth(binned/binSize,[smooth,kernel_size]); % ONLY CHANGE I MADE HERE IS SPECIFYING kernel size TO AVOID WARNING
	frequency = [t f];
